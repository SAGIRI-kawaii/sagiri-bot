...
async def check_group_data_init(group_list: list) -> None:
    ...
    for i in group_list:
        ...
        if i.id not in group_id:
            ...
            `real`,r18,search,imgPredict,yellowPredict,imgLightning,music,speakMode,switch,forbiddenCount) 
            VALUES 
            (%d,'%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,'%s','%s','%s',0)
            """ % (i.id, i.name, True, True, True, True, 6, False, False,
                   False, False, False, False, False, False, "off", "normal", "online")
