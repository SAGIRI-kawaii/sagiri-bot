...
class StylePictureGeneraterHandler(AbstractHandler):
    ...
    async def gosencho_en_hoshi_style_image_generator(group: Group, member: Member, message: MessageChain) -> MessageItem:
        try:
            ...
            return MessageItem(MessageChain.create([Plain(text="参数非法！使用格式：5000兆 text1 text2")]), Normal(GroupStrategy()))
    ...
    async def youtube_style_image_generator(group: Group, member: Member, message: MessageChain) -> MessageItem:
        ...
        try:
            ...
            return MessageItem(MessageChain.create([Plain(text="格式错误！使用方法：yt left right!")]), QuoteSource(GroupStrategy()))
        try:
            return MessageItem(await YoutubeStyleUtils.make_yt_style_logo(left_text, right_text), Normal(GroupStrategy()))
        except OSError as e:
            if "[Errno 22] Invalid argument:" in str(e):
                return MessageItem(MessageChain.create([Plain(text="非法字符！")]), QuoteSource(GroupStrategy()))
