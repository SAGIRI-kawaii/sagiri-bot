    ...
    async def handle(self, app: GraiaMiraiApplication, message: MessageChain, group: Group, member: Member):
        ...
        if re.match(r"添加(setu|setu18|real|realHighq|wallpaper|sketch)图片(\[图片])+", message.asDisplay()):
            if not user_permission_require(group, member, 2):
                set_result(message, MessageItem(MessageChain.create([Plain(text="你没有权限，爬！")]), Normal(GroupStrategy())))
            ...
            if path := get_config(f"{image_type}Path"):
                if os.path.exists(path):
                    try:
                        await self.add_image(path, message.get(Image))
                    except Exception as e:
                        logger.error(traceback.format_exc())
                        set_result(
                            message,
                            MessageItem(MessageChain.create([Plain(text="出错了呐~请查看日志/控制台输出！")]), Normal(GroupStrategy()))
                        )
                    set_result(
                        message,
                        MessageItem(
