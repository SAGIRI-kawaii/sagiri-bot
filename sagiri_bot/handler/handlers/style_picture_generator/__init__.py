...
class StylePictureGenerator(object):
    ...
    def gosencho_en_hoshi_style_image_generator(message: MessageChain) -> MessageChain:
        try:
            ...
            try:
                img_byte = BytesIO()
                GoSenChoEnHoShiStyleUtils.genImage(
...
class GoSenChoEnHoShiStyleUtils:
    ...
    def genBaseImage(width=1500, height=150):
        ...
        result = {
            "downerSilver": IMG.fromarray(np.uint8(downerSilverArray)).crop(
                (0, 0, width, height)
            ),
            "gold": IMG.fromarray(np.uint8(goldArray)).crop((0, 0, width, height)),
            "red": IMG.fromarray(np.uint8(redArray)).crop((0, 0, width, height)),
            "strokeRed": IMG.fromarray(np.uint8(strokeRedArray)).crop(
                (0, 0, width, height)
            ),
            "silver2": IMG.fromarray(np.uint8(silver2Array)).crop(
                (0, 0, width, height)
            ),
            "strokeNavy": IMG.fromarray(np.uint8(navyArray)).crop(
                (0, 0, width, height)
            ),  # Width: 7
            "baseStrokeBlack": IMG.new("RGBA", (width, height), rgb(0, 0, 0)).crop(
                (0, 0, width, height)
            ),
            # Width: 17
            "strokeBlack": IMG.new("RGBA", (width, height), rgb(16, 25, 58)).crop(
                (0, 0, width, height)
            ),  # Width: 17
            "strokeWhite": IMG.new("RGBA", (width, height), rgb(221, 221, 221)).crop(
                (0, 0, width, height)
            ),
            # Width: 8
            "baseStrokeWhite": IMG.new(
    ...
    def genImage(
        ...
        font_upper = ImageFont.truetype(
            f"{os.getcwd()}/statics/fonts/STKAITI.TTF", _round(height / 3)
        )
...
class PornhubStyleUtils:
    ...
    def create_left_part_img(text: str, font_size: int):
        font = ImageFont.truetype(
            f"{os.getcwd()}/statics/fonts/ArialEnUnicodeBold.ttf", font_size
        )
