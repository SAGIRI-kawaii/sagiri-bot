...
async def petpet(image: Union[int, str], flip=False, squish=0, fps=20) -> MessageChain:
    ...
    gif_frames = [
        await make_frame(avatar, i, squish=squish, flip=flip)
        for i in range(5)
    ]
...
async def kiss(operator_image: Union[int, str], target_image: Union[int, str]) -> MessageChain:
    """
...
async def rub(operator_image: Union[int, str], target_image: Union[int, str]) -> MessageChain:
