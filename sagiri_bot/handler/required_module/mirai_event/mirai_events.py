...
with open(
    str(Path(os.path.dirname(__file__)) / "event_config.yaml"), "r", encoding="utf-8"
) as f:
...
async def member_unmute_event(app: Ariadne, group: Group, event: MemberUnmuteEvent):
    try:
        if not await group_setting.get_setting(group, Setting.switch):
            return None
        await app.send_message(
            event.member.group, MessageChain(f"啊嘞嘞？{event.member.name}被放出来了呢~")
        )
    except AccountMuted:
        pass
...
async def member_leave_event_kick(
    app: Ariadne, group: Group, event: MemberLeaveEventKick
):
    try:
        if not await group_setting.get_setting(group, Setting.switch):
            return None
        await app.send_message(
            event.member.group,
            MessageChain(f"<{event.member.name}> 被 <{event.operator.name}> 🐏辣~"),
        )
    except AccountMuted:
        pass
...
async def member_special_title_change_event(
    app: Ariadne, group: Group, event: MemberSpecialTitleChangeEvent
):
    try:
        if not await group_setting.get_setting(group, Setting.switch):
            return None
        await app.send_message(
            event.member.group,
            MessageChain(
                f"啊嘞嘞？{event.member.name}的群头衔从{event.origin}变成{event.current}了呐~"
            ),
        )
    except AccountMuted:
        pass
...
async def member_permission_change_event(
    app: Ariadne, group: Group, event: MemberPermissionChangeEvent
):
    try:
        if not await group_setting.get_setting(group, Setting.switch):
            return None
        await app.send_message(
            event.member.group,
            MessageChain(f"啊嘞嘞？{event.member.name}的权限变成{event.current}了呐~"),
        )
    except AccountMuted:
        pass
...
async def bot_leave_event_kick(app: Ariadne, event: BotLeaveEventKick):
    ...
    await app.send_friend_message(
        config.host_qq, MessageChain(f"呜呜呜主人我被踢出{event.group.name}群了")
    )


async def group_name_change_event(
    app: Ariadne, group: Group, event: GroupNameChangeEvent
):
    try:
        if not await group_setting.get_setting(group, Setting.switch):
            return None
...
async def group_allow_anonymous_chat_event(
    app: Ariadne, group: Group, event: GroupAllowAnonymousChatEvent
):
    try:
        if not await group_setting.get_setting(group, Setting.switch):
            return None
...
async def group_allow_confess_talk_event(
    app: Ariadne, group: Group, event: GroupAllowConfessTalkEvent
):
    try:
        if not await group_setting.get_setting(group, Setting.switch):
            return None
...
async def group_allow_member_invite_event(
    app: Ariadne, group: Group, event: GroupAllowMemberInviteEvent
):
    try:
        if not await group_setting.get_setting(group, Setting.switch):
            return None
        await app.send_message(
            event.group,
            MessageChain(
                f"现在{'允许邀请成员加入辣！快把朋友拉进来玩叭！' if event.current else '不允许邀请成员加入辣！要注意哦~'}"
            ),
        )
    except AccountMuted:
        pass
...
async def member_card_change_event(
    app: Ariadne, group: Group, event: MemberCardChangeEvent
):
    try:
        if not await group_setting.get_setting(group, Setting.switch):
            return None
        if (
            ...
            if event.operator:
                ...
                await app.send_message(
                    group, MessageChain(f"啊嘞嘞？{event.origin}的群名片改为{event.current}了呢！")
                )
    except AccountMuted:
        pass
...
async def group_recall_event(app: Ariadne, group: Group, event: GroupRecallEvent):
    ...
    if (
        ...
        and event.author_id != config.bot_qq
    ):
        try:
            ...
            author_name = (
                "自己" if event.operator.id == event.author_id else author_member.name
            )
            resend_msg = MessageChain(
                f"{event.operator.name}偷偷撤回了{author_name}的一条消息哦：\n\n"
            ).extend(revoked_msg)
...
async def member_honor_change_event(
    app: Ariadne, group: Group, event: MemberHonorChangeEvent
):
    await app.send_message(
        group,
        MessageChain(
