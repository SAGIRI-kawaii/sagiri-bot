...
async def saya_manager(app: Ariadne, message: MessageChain, group: Group, member: Member, source: Source):
    if message.display.strip() == "已加载插件":
        ...
        if target.isdigit():
            ...
            if not 0 <= int(target) - 1 < len(keys):
                return await app.send_group_message(group, MessageChain("错误的编号！请检查后再发送！"), quote=source)
            ...
            for lchannel in loaded_channels.keys():
                ...
                return await app.send_group_message(group, MessageChain("错误的名称！请检查后再发送！"), quote=source)
        return await app.send_group_message(
            group,
            MessageChain([
        ...
        if not await user_permission_require(group, member, 3):
            return await app.send_group_message(group, MessageChain("爬，权限不足"), quote=source)
        ...
        if not result:
            return await app.send_group_message(group, MessageChain("非预期回复，进程退出"), quote=source)
        elif result == "是":
            result = load_channel(channel)
            if result:
                return await app.send_group_message(group, MessageChain(f"发生错误：{result[channel]}"), quote=source)
            else:
                return await app.send_group_message(group, MessageChain("加载成功"), quote=source)
        else:
            return await app.send_group_message(group, MessageChain("进程退出"), quote=source)
        ...
        await app.send_message(
            group, MessageChain(f"你确定要{message.display[0]}载插件 `{channel._name}` 吗？（是/否）")
        )
        ...
        if not result:
            ...
            result = unload_channel(
                channel_path) if load_type == "unload" else reload_channel(channel_path)
            if result:
                return await app.send_group_message(
                    group, MessageChain(f"发生错误：{result[channel_path]}"), quote=source
                )
            else:
                return await app.send_group_message(group, MessageChain(f"{message.display[0]}载成功"), quote=source)
        else:
            return await app.send_group_message(group, MessageChain("进程退出"), quote=source)
        ...
        return await app.send_group_message(
            group, MessageChain(f"插件{channel_path}已{message.display[:2]}！"), quote=source
        )
