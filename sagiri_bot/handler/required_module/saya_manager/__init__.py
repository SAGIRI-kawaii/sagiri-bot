    ...
    async def handle(
        ...
        if message.asDisplay().strip() == "已加载插件":
            ...
            for cube in channel.content:
                for twilight in cube.metaclass.inline_dispatchers:
                    fwd_nodes.append(
                        ForwardNode(
                            target=app.account,
                            name="纱雾酱",
                            time=datetime.now() + timedelta(seconds=15) * (offset + 1),
                            message=MessageChain(twilight.get_help()),
                        )
                    )
            await app.sendGroupMessage(
                group, MessageChain.create([Forward(nodeList=fwd_nodes)])
            )
            return
            ...
            if not await user_permission_require(group, member, 3):
                return MessageItem(
                    MessageChain.create([Plain(text="爬，权限不足")]), QuoteSource()
                )
            ...
            await app.sendMessage(
                group,
                MessageChain.create([Plain(text=f"你确定要加载插件 `{channel}` 吗？（是/否）")]),
            )
            ...
            if not result:
                return MessageItem(
                    MessageChain.create([Plain(text="非预期回复，进程退出")]), QuoteSource()
                )
            elif result == "是":
                result = SayaManager.load_channel(channel)
                if result:
                    return MessageItem(
                        MessageChain.create([Plain(text=f"发生错误：{result[channel]}")]),
                        QuoteSource(),
                    )
                else:
                    return MessageItem(
                        MessageChain.create([Plain(text="加载成功")]), QuoteSource()
                    )
            else:
                return MessageItem(
                    MessageChain.create([Plain(text="进程退出")]), QuoteSource()
                )
            ...
            await app.sendMessage(
                ...
                MessageChain.create(
                    [
                        Plain(
                            text=f"你确定要{message.asDisplay()[0]}载插件 `{channel._name}` 吗？（是/否）"
                        )
                    ]
                ),
            )
...
async def saya_manager(
    ...
    if result := await SayaManager.handle(app, message, group, member):
        await MessageSender(result.strategy).send(
            app, result.message, message, group, member
        )
