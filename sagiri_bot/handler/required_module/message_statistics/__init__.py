...
@channel.use(
    ListenerSchema(
        ...
        inline_dispatchers=[
            Twilight([FullMatch("消息量统计"), ArgumentMatch("-group", action="store_true", optional=True) @ "group_only"])
        ],
        decorators=[
...
async def message_statistic(
    app: Ariadne, group: Group, group_only: ArgResult
):
    if group_only.matched:
        return await app.send_group_message(group, await get_message_statistic(group=group))
    else:
...
def get_mapping(time_tags: List[str], talk_num: List[int], group_name: str = '') -> bytes:
    if group_name:
    ...
    plt.title(f"{group_name}消息量统计（24h内共 {sum(talk_num)} 条）", fontsize=36, fontproperties=zhfont1)
