...
async def color_card(
    ...
    if image.matched:
        ...
        image_bytes = await get_user_avatar(at.result.target if at.matched else qq.result.display.strip())
    else:
        try:
    ...
    result = await asyncio.to_thread(draw, image_bytes, mode, size)
    bytes_io = BytesIO()
...
def draw_ellipse(image, bounds, width=1, antialias=4):
    mask = PIL.Image.new(size=[int(dim * antialias) for dim in image.size], mode="L", color="black")
