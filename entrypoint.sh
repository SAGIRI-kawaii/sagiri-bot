...
while :
do
  echo -e "\033[32m[sagiri-bot-docker]当前目录为(容器内部运行目录): $PWD \033[0m"
  stillRunning=$(ps -ef |grep "python main.py" |grep -v "grep")
  if [ "$stillRunning" ] ; then
    echo -e "\033[31m[sagiri-bot-docker]发现通过了其他的方式启动了sagiri-bot\033[0m" 
    ...
    then
      ...
      echo -e "\033[31m[sagiri-bot-docker]也许你应该手动检查下"
      ...
      /bin/bash
      echo -e "\033[32m[sagiri-bot-docker]检测到维护退出,重置计数器,并且尝试重新启动sagiri-bot\033[0m"
    ...
    then
      echo -e "\033[31m[sagiri-bot-docker]连续运行失败次数达到10次以上,进入shell模式,请通过 \033[36m docker attach sagiri-bot(sagiri-bot为容器名) \033[0m 进入容器shell手动维护,结束维护输入\033[36m exit \033[0m \033[0m"
      /bin/bash
      echo -e "\033[32m[sagiri-bot-docker]检测到维护退出,重置计数器,并且尝试重新启动sagiri-bot\033[0m"
      cnt_failed_running=0
    fi
