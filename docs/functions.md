...
| 二次元线稿图 | 在群中发送 线稿 即可 | 所有人 | 是 | 开关跟随二次元图功能即setu |
...
| ~~获取时间~~ | ~~在群中发送 `几点了` 即可~~ | ~~所有人~~ | ~~否~~ | ~~需要提前配置背景以及对应时间的图片（0:00 ~ 23:59）~~ 已移除 |
...
| 设置个人词云模板 | 在群中发送 `设置总结模板 %Image%` 即可 | 所有人 | 是 | `%Image%` 为要设置的模板（图片） |
...
| 获取今日群内发言排名图表 | 在群中发送 `今日发言图表` 即可 | 所有人 | 否 |  |
| xml涩图 | 在群中发送 `xml %keyword%` 即可 | 所有人 | 是 | `%keyword%` 为要搜索的关键词 |
| 成就系统 | 在群中正常使用即可，完成成就会有提示 | 所有人 | 是 | 目前仅有聊天和涩图成就 |
| 获取成就 | 在群中发送 `我的成就` 即可 | 所有人 | 否 |  |